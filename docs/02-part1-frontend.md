...
│  │  筛选: [全部▾]  排序: [最新▾]                        │  │
│  │  ┌────────────────────────────────────────────────┐ │  │
│  │  │ ✅ 博主A | 标准 | 06-12 10:00 | [画像] [调配室] │ │  │
│  │  │ ✅ 博主B | 深度 | 06-11 14:00 | [画像] [调配室] │ │  │
...
- 自动轮询 `GET /api/v1/analysis/list?status=pending,collecting,analyzing,waiting_tags,processing`
- 每条显示：博主昵称/头像、分析等级、当前步骤、进度条。
- **状态分流逻辑**:
  - 当状态为 `waiting_tags` 时，进度条右侧展示 `[前往筛选标签]` 按钮，点击跳转 `/tags/:taskId`。
  - 当状态为 `processing` 时，进度条右侧展示 `[进入调配室]` 按钮，点击跳转 `/recommend/:taskId` 观看沉浸式生成状态。
  - 当状态为 `pending`, `collecting`, `analyzing` 时，展示 `[查看进度现场]` 按钮，点击跳转任务进度页 `/task/:taskId` 观看实时日志流。
- 轮询间隔：10 秒

### 2.4 历史记录列表

- 调用 `GET /api/v1/analysis/list?status=completed,failed&page=1&page_size=20` (仅展示彻底完成或失败的任务)
- 支持按状态筛选（全部/已完成/失败）
- 每条显示：博主昵称/头像、分析等级、完成时间、操作按钮
- 操作按钮：
  - **[查看画像]** → 跳转画像报告页 `/report/:taskId`
  - **[进入调配室]** → 跳转到三栏工作台 `/recommend/:sessionId`
  - **[删除]** → 确认弹窗后删除
