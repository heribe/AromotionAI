...
| `POST` | `/api/v1/fragrance/{session_id}/chat` | 追问对话 |
...
> [!NOTE]
> 以下配置项在「高级自定义」模式下可由调香师逐一调整。具体 UI 交互需要在实现时进一步细化。
...
| **子评论** | `enabled` | 是否采集评论下的回复 | ⚠️ 需确认子评论 API |
| | `count` | 每条评论采集多少条回复 | |
...
| # | 问题 | 所在文档 | 优先级 |
|---|---|---|---|
| 1 | 分析配置的高级自定义参数列表的最终确认 | Part1 后端 | 高 |
| 2 | 组图分析 vs 单图分析的精度对比与策略 | Part1 后端 | 中 |
| 3 | 子评论 API 的可行性验证 | Part1 后端 | 中 |
| 4 | 冰山理论的具体分层映射规则 | Part2 后端 | 低（先 mock） |
| 5 | 冰山理论的探索方向建议 | Part2 后端 | 低 |
| 6 | Part2 对话是否需要流式输出（SSE/Stream） | Part2 前后端 | 中 |
| 7 | 多博主对比功能的数据结构设计 | 全局 | 低（预留） |
| 8 | 是否需要支持调香师自定义标签（不在预设标签体系内的） | Part1 前端 | 中 |
