...
class AnalysisService:
    ...
    async def run_analysis(self, task_id: str) -> ProfileReport:
        ...
        # 实例化采集器
        collector = DouyinCollector(db=self.db, test_mode=test_mode)
        media_processor = MediaProcessor(test_mode=test_mode)
        ...
        try:
            ...
            # 推送 progress 事件
            await self.task_manager.emit(task_id, "progress", {
                "status": "collecting",
                "progress": 5,
                "current_step": "正在采集博主资料"
            })
            ...
            posts = await collector.get_blogger_posts(blogger.platform_uid, count=collect_count, task_id=task_id)
            ...
            # 推送 step_complete
            await self.task_manager.emit(task_id, "step_complete", {
                "step": "博主资料",
                ...
                if test_mode == "mock":
                    ...
                    if post.cover_url:
                        ...
                        try:
                            ...
                            logger.warning(f"Download cover failed for {post.aweme_id}: {download_err}. Fallback to mock cover.")
                            img = Image.new("RGB", (10, 10), color=(255, 0, 0))
                            img.save(final_path, "JPEG")
                            post.local_cover_path = final_path
                        finally:
            ...
            all_comments = []
            for post in selected_posts:
                comments = await collector.collect_comments(post.aweme_id, count=per_post_count, task_id=task_id)
            ...
            if enable_commenter_analysis:
                ...
                import random
                provinces = ["广东", "北京", "上海", "浙江", "江苏", "四川", "山东", "辽宁", "吉林", "黑龙江", "河北", "河南"]
                cities = ["深圳", "北京", "上海", "杭州", "南京", "成都", "济南", "沈阳", "长春", "哈尔滨", "石家庄", "郑州"]

                for tc in target_commenters:
                    avatar_url = None
                    if isinstance(tc.raw_data, dict):
                        user_data = tc.raw_data.get("user") or {}
                        avatar_url = user_data.get("avatar_url") or user_data.get("avatar_thumb", {}).get("url_list", [None])[0]
            ...
            if commenter_profiles:
                avatar_paths = [cp.avatar_url for cp in commenter_profiles if cp.avatar_url and os.path.exists(cp.avatar_url)]
                if avatar_paths:
                    grid_path = os.path.join(settings.BASE_DIR, "data", "media", "grids", f"grid_{task_id}.jpg")
                    os.makedirs(os.path.dirname(grid_path), exist_ok=True)
                    cell_count = len(avatar_paths)
                    media_processor.create_grid_image(avatar_paths, grid_path, grid_size=cell_count)
                    fan_visual_results = await visual_analyzer.analyze_grid(grid_path, person_count=cell_count)

            await self.task_manager.emit(task_id, "step_complete", {
                "step": "内容分析",
                "summary": "AI 内容视觉与评论语义分析完成"
            })
            ...
            await self.task_manager.emit(task_id, "complete", {
                "task_id": task_id,
                "report_id": report.id
            })
            ...
            await self.task_manager.emit(task_id, "error", {
                "status": "failed",
